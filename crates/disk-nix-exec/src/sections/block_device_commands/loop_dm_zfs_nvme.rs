...
fn zfs_create_wrapper_argv(target: &str, property_assignments: &[String]) -> Vec<String> {
    let mut argv = vec![
        "bash".to_string(),
        "-c".to_string(),
        "target=\"$1\"; shift; interactive=0; for arg in \"$@\"; do if [ \"$arg\" = keylocation=prompt ]; then interactive=1; fi; done; zfs_values_match() { property=\"$1\"; desired=\"$2\"; current=\"$3\"; if [ \"$current\" = \"$desired\" ]; then return 0; fi; case \"$property:$desired:$current\" in acltype:posixacl:posix) return 0 ;; esac; return 1; }; verify_properties() { while [ \"$#\" -gt 0 ]; do if [ \"$1\" = -o ]; then shift; assignment=\"$1\"; property=\"${assignment%%=*}\"; desired=\"${assignment#*=}\"; current=\"$(zfs get -H -p -o value \"$property\" \"$target\" 2>/dev/null || true)\"; if ! zfs_values_match \"$property\" \"$desired\" \"$current\"; then echo \"existing ZFS target $target has $property=$current, expected $desired\" >&2; return 1; fi; fi; shift || true; done; }; if zfs list -H \"$target\" >/dev/null 2>&1; then verify_properties \"$@\"; exit \"$?\"; fi; if [ \"$interactive\" -eq 1 ]; then if [ ! -r /dev/tty ] || [ ! -w /dev/tty ]; then echo \"ZFS keylocation=prompt requires a controlling terminal; rerun from a TTY or render a script with --script-out\" >&2; exit 2; fi; if zfs create \"$@\" \"$target\" </dev/tty >/dev/tty 2>&1; then exit 0; else status=\"$?\"; fi; else if zfs create \"$@\" \"$target\"; then exit 0; else status=\"$?\"; fi; fi; if zfs list -H \"$target\" >/dev/null 2>&1 && verify_properties \"$@\"; then exit 0; fi; exit \"$status\"".to_string(),
