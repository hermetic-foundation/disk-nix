...
subvolume, VDO, MD RAID, multipath, and zvol lifecycle declarations,
higher-layer lifecycle collections, snapshot declarations, supported operation
names, apply policy fields, and NixOS activation helper fields such as
`probeCurrent`, `failOnBlocked`, `scriptOut`, and `reportOut`.
