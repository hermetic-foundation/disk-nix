...
- Explicitly enabled smoke integration harnesses for loop devices, Btrfs,
  ...
  reporting for both a layered LVM-plus-filesystem apply failure, an LVM grow
  failure, and a ZFS rollback failure, plus NVMe namespace create, grow, attach, detach, and
...
- Broader destructive and failure-path integration tests beyond the smoke
  ...
  recovery beyond the synthetic LVM-plus-filesystem, LVM grow, swap label,
