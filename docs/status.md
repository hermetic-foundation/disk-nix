...
- Current-topology reconciliation suppresses safe no-op grow, shrink, iSCSI
  ...
  VDO start, and property actions when the graph proves they are already
  satisfied and no warning diagnostics are present; inactive LVM objects,
  inactive LUKS mappers, non-normal VDO operating modes, mount source
  mismatches, remount option differences, export client/option differences,
  and known iSCSI targets without logged-in sessions remain actionable
  warnings.
