...
- Explicitly enabled smoke integration harnesses for loop devices, Btrfs,
  ...
  backing-file rescan failure, backing-file grow failure, device-mapper rename failure,
...
and source-to-mount edges, plus a SAS enclosure fixture
...
- Broader destructive and failure-path integration tests beyond the smoke
  ...
  backing-file rescan, backing-file grow, device-mapper rename, ZFS dataset rename, Btrfs snapshot clone,
