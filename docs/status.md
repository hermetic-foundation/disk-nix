...
- Explicitly enabled smoke integration harnesses for loop devices, Btrfs,
  ...
  failures, Linux tgt create, attach, and destroy failures, multipath resize
  and replacement failures, MD RAID member replacement failure, LUKS open
  ...
  snapshot clone failure, ZFS snapshot clone failure, LVM VG rename failure,
  iSCSI logout and login failures, LVM cache attach and detach failures, VDO
  grow failure, VDO and bcache property mutation failures, and an LVM cache
  property mutation failure, without touching real block devices.
...
- Broader destructive and failure-path integration tests beyond the smoke
  ...
  attach, target-side LUN tgt destroy, multipath resize, multipath replace, MD
