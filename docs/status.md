...
- Explicitly enabled smoke integration harnesses for loop devices, Btrfs,
  ...
  replacement, and rescan failures, VDO create/rescan/grow/start/stop/remove
  failures, bcache replacement failure, VDO and bcache property mutation
  failures, bcache rescan failure, and an LVM cache property mutation failure,
...
- Broader destructive and failure-path integration tests beyond the smoke
  ...
  LVM cache replacement, LVM cache rescan, VDO create, VDO rescan, VDO grow,
