...
- Current-topology reconciliation suppresses safe no-op grow, shrink, iSCSI
  ...
  VDO stop, MD assemble, ZFS pool import, Btrfs subvolume create, LVM
  ...
  with different or unknown current size, Btrfs subvolume create targets that
  match a non-subvolume path,
