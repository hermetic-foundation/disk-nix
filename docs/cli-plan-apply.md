...
This document contains the detailed planner, apply, rollback, and validation contracts for the CLI. Use [CLI](cli.md) for command discovery and focused read-only views.
...
failures can replay declared `rollbackValue`.

NFS mount verification failures can replay `umount <mountpoint>`. iSCSI login
verification failures can replay the matching `iscsiadm --logout`, and
target-side LUN property failures can replay provider-specific declared
`rollbackValue`.

NFS unmount/unexport, iSCSI logout, host or target LUN growth, target LUN
attach/detach, remote export lifecycle, and LUN topology boundaries remain
refused/operator-only without stronger initiator, target, active-consumer, and
backing-store proof.
...
The execution library materializes deterministic evidence IDs from the failed
execution report and fresh probe ID unless callers provide explicit evidence
through `replay_proven_safe_rollback_recipe_with_topology_evidence`.
