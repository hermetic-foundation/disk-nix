...
- [ ] **Partial:** Broader destructive and failure-path integration tests are
  ...
  XFS grow, Btrfs scrub, Btrfs rebalance, filesystem trim, swap label,
  device-mapper rename, ZFS dataset rename, Btrfs snapshot clone, ZFS snapshot clone, LVM VG rename, ZFS rollback, NVMe namespace create, NVMe namespace
