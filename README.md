...
until the LUKS header device is explicit. Current-topology probing reconciles
LUKS label, subsystem, and UUID declarations against probed LUKS identity and
`cryptsetup.luks-*` header metadata before suppressing already-satisfied
updates. Logical LUKS declaration keys can declare the concrete mapper name
