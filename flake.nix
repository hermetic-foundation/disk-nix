{
  ...
  outputs =
    ...
    let
      ...
      perSystem = forAllSystems (
        ...
        let
          ...
          format = import ./nix/flake/formatter.nix { inherit pkgs self; };
          formatProgram = format.formatter;
          diskNix = import ./nix/flake/package.nix { inherit pkgs self; };
            ...
            testScript = ''
              ...
              for spec in [
                ...
                "zfs.json",
                  ...
                  machine.succeed(f"cp ${./examples/disko}/{spec} /tmp/disko-kernel-examples/{spec}")
              machine.succeed(
          ...
          nixosModuleTests = import ./nix/flake/nixos-module-tests.nix { inherit pkgs self; };
        ...
        in
        {
          ...
          checks = {
            ...
            integrationMdraidSmoke = pkgs.runCommand "disk-nix-integration-mdraid-smoke-check" { } ''
              ...
              ${pkgs.gnugrep}/bin/grep -q 'failedReattach' ${./scripts/integration-mdraid-smoke.sh}
    ...
    in
    {
      ...
      nixosModules = import ./nix/flake/nixos-modules.nix { inherit self; };
